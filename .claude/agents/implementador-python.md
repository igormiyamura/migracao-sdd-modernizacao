...
2. Escreva `src/<slug>/config.py`: um modelo `pydantic` cobrindo todo valor de configuração identificado no discovery/plano (caminhos, endpoints, tolerâncias, listas de destinatários) — nada disso vai hardcoded na lógica.
3. Escreva a lógica seguindo a estrutura que o plano definiu (passo 3 de `.claude/agents/planejador.md`): `logica.py` único se o plano optou por isso, ou um subpacote `logica/` com um arquivo por `modulo.id` do discovery — uma função por etapa em qualquer um dos dois casos, implementando exatamente a `logica`/`regras_negocio` capturada, inclusive os casos de borda das condições. Se o plano sinalizou uma questão em aberto ainda não resolvida com o analista, pare e pergunte antes de decidir por conta própria.
4. Escreva `src/<slug>/fluxo.py`: orquestra a leitura das entradas via `conectores/` (nomes lógicos de credencial vindos de `config.py`), aplica a lógica na ordem de dependência entre módulos definida no plano, escreve as saídas via `conectores/`.
...
`uv run ruff check`, `uv run mypy` e `uv run pytest` passam sem erro, toda etapa de todo módulo do discovery tem função correspondente na lógica, e nenhuma chamada a `pywin32`/`smbclient`/`O365`/`msal`/`requests` aparece fora de `conectores/`.
